...
export function mountAnchoLateral(contenedor) {
	/** Ancho al que volver al desplegar. */
	let anchoAbierta = POR_DEFECTO;
	let colapsada = false;
	function aplicar() {
		document.documentElement.style.setProperty("--sidebar-width", `${colapsada ? ANCHO_COLAPSADA : anchoAbierta}px`);
	...
	function guardar() {
		globalThis.localStorage?.setItem(ALMACEN, colapsada ? COLAPSADA : String(anchoAbierta));
	}
	function restaurar() {
