...
export function mountPlayerBar(contenedor, opciones) {
	...
	*/
	const alMenuDeLaActual = (e) => {
		const pista = estado?.track;
		if (!pista) return;
		e.preventDefault();
		abrirMenu(e.clientX, e.clientY, opcionesDePista(pista, {
	...
	*/
	...
	function pintarRepeticion() {
		...
		const etiqueta = modo === "off" ? t("player.repeat_off") : modo === "queue" ? t("player.repeat_queue") : t("player.repeat_track");
	...
	function pintarPosicion(pos, dur) {
		transcurrido.textContent = duracion(pos);
		if (arrastrando) return;
		const fraccion = dur > 0 ? pos / dur : 0;
		rango.value = String(Math.round(fraccion * 1e3));
	...
	async function alternarAleatorio() {
		pintar(await player.setShuffle(!(estado?.shuffle ?? false)));
	}
	async function rotarRepeticion() {
	...
	// Teclado (Tab + flechas/Home/End): no hay pointerdown, así que sin esto
	// "change" llega con arrastrando aún en false y alSoltarBarra no hace nada.
	rango.addEventListener("keydown", () => {
		arrastrando = true;
	});
	rango.addEventListener("input", () => {
		...
		const destino = Number(rango.value) / 1e3 * dur;
	...
	const alSoltarBarra = () => {
		...
		arrastrando = false;
		const dur = estado?.durationMs ?? 0;
		void player.seek(Math.round(Number(rango.value) / 1e3 * dur)).then(pintar);
	};
	...
	megusta.addEventListener("click", () => {
		const pista = estado?.track;
		if (!pista) return;
		void import("../ipc/client.js").then(({ library }) => library.setFavorite(pista.id, !pista.isFavorite).then(refrescar));
	});
	// ── Sincronización ──────────────────────────────────────────────────────
	function refrescar() {
	...
	*/
	const alEspacio = (e) => {
		...
		const escribiendo = activo instanceof HTMLInputElement || activo instanceof HTMLTextAreaElement || activo instanceof HTMLSelectElement || activo instanceof HTMLElement && activo.isContentEditable;
		const yaLoManeja = activo instanceof HTMLButtonElement || activo instanceof HTMLAnchorElement;
