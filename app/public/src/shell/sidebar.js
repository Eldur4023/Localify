...
import { arrastrable, reordenable, zonaDeReordenacion, zonaDeSoltado, TIPO_PISTAS, TIPO_PLAYLIST } from "../ui/dnd.js";
export function mountSidebar(contenedor, router) {
	...
	async function crear() {
		const respuesta = await pedirTextoConImagen({
			...
			maxLength: 100,
			imagen: {
				etiqueta: t("playlist.cover_choose"),
				elegir: () => api.pickImage()
			}
		});
		if (!respuesta) return;
		try {
	...
	*/
	...
	function pintar(items) {
		...
		for (const p of ordenActual) {
			...
			// nadie se lo diga.
			soltados.push(zonaDeSoltado(a, TIPO_PISTAS, async (ids) => {
			...
			a.addEventListener("contextmenu", (e) => {
				e.preventDefault();
				abrirMenu(e.clientX, e.clientY, opcionesDePlaylist(p, {
					alCambiar: refrescar,
					alBorrar: () => {
						refrescar();
	...
	*/
	const dejarReordenPlaylists = zonaDeReordenacion(lista, (destino) => destino instanceof Element ? destino.closest(".sidebar__item[data-playlist-id]") : null, (fila) => [...lista.querySelectorAll(".sidebar__item[data-playlist-id]")].indexOf(fila), async (playlistId, indice) => {
		const desde = ordenActual.findIndex((p) => p.id === playlistId);
		if (desde < 0) return;
		const anclas = [...lista.querySelectorAll(".sidebar__item[data-playlist-id]")];
		...
		if (movida && nodoMovido) {
			...
			const restantes = anclas.filter((_, i) => i !== desde).map((a) => a.closest("li"));
