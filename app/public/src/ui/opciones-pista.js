...
*/
async function nuevaCon(trackId, alCambiar) {
...
async function destinos(pista, ctx) {
...
export function opcionesDePista(pista, ctx) {
	const menu = [
		{
			clave: "play",
		...
		},
		{
			...
			ejecutar: () => void library.setFavorite(pista.id, !pista.isFavorite).then(() => ctx.alCambiar?.())
		}
	];
	...
	menu.push({
		...
		icono: "close",
		ejecutar: async () => {
			const seguro = await confirmar(t("menu.delete_track"), t("menu.delete_track_do"), t("menu.delete_track_confirm"));
			if (!seguro) return;
