...
export async function renombrar(p, reacciones) {
...
*/
export async function describir(p, reacciones) {
...
* donde salió.
*/
export async function cambiarFoto(p, reacciones) {
...
export async function quitarFoto(p, reacciones) {
	await playlists.clearCover(p.id);
	reacciones.alCambiar?.();
}
/** Pide confirmación y borra. */
export async function borrar(p, reacciones) {
	const seguro = await confirmar(t("playlist.delete_confirm", { name: p.name }), t("playlist.delete"));
	if (!seguro) return;
	await playlists.remove(p.id);
	reacciones.alBorrar?.();
}
/** Menú contextual de una playlist. */
export function opcionesDePlaylist(p, reacciones = {}) {
	const menu = [
		{
			clave: "rename",
