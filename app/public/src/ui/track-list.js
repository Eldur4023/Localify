...
import { comienzoReciclable } from "./cards.js";
...
function fechaCorta(segundos) {
	...
	return formato.format(new Date(segundos * 1e3));
}
/**
...
*/
function pintarDisponibilidad(el, a) {
	switch (a.kind) {
		case "downloading":
...
export function mountTrackList(contenedor, opciones) {
	...
	*/
	...
	const lista = mountVirtualList(cuerpo, {
		rowHeight: ALTO_FILA,
		createRow() {
			...
			if (opciones.playlistId) {
				desmontadores.push(reordenable(fila, () => fila.dataset["entryId"] ?? ""));
			}
	...
	*/
	...
	function abrir(indice, x, y) {
		const pista = lista.items[indice];
		if (!pista) return;
		marcar(indice);
		abrirMenu(x, y, opcionesDePista(pista, {
	...
	const alTeclear = (e) => {
		...
		switch (e.key) {
			...
			case "ContextMenu": {
				e.preventDefault();
				const fila = lista.el.querySelector(`#pista-${activa}`);
	...
	*/
	...
	function filaDe(trackId) {
		return cuerpo.querySelector(`[data-track-id="${CSS.escape(trackId)}"]`);
	}
